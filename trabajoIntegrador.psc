Proceso trabajoIntegrador
    ...
    Si usuario = usuario_correcto y contrasena = contrasena_correcta Entonces
        logueado <- Verdadero
	...
	ticket <- ticket + "Hamburguesa: $" + ConvertirATexto(hamburguesa)
	
	Si extraCarne = 'S' o extraCarne = 's' Entonces
		ticket <- ticket + " Carne extra: $500"
	FinSi
	
	Si papas = 'S' o papas = 's' Entonces
		ticket <- ticket + "Papas fritas: $250"
	FinSi
	
	Si gaseosa = 'S' o gaseosa = 's' Entonces
		ticket <- ticket + "Gaseosa: $1000"
	FinSi
	
	// Aplicar descuento si está logueado
	Si logueado Entonces
		ticket <- ticket + "Total: $" + convertirATexto(total)
		total <- total * (1 - descuento)
		ticket <- ticket + " + Descuento del 10% --> Total: $" + convertirATexto(total)
	SiNo
		ticket <- ticket + "Total: $" + convertirATexto(total)
	FinSi
	
	ticket <- ticket + "Tiempo de espera: " + convertirATexto(tiempoEspera) + " minutos"
	ticket <- ticket + "---------------------------"
